<form action="{{ $model->exists ? route('ga.room.update', base64_encode($model->id)) : route('ga.room.store') }}" method="POST">
    ...
    @if ($model->exists)
        ...
        <input type="hidden" id="gedung_value" value={{ $model->room->gedung }}>
        <input type="hidden" id="room_value" value={{ $model->room->code}}>
    @endif
                ...
                <div class="col-md-8">
                    <select name="gedung_code" id="gedung_code" class="form-control select2bs4" style="width: 100%" {{ $disabled }}>
                        ...
                        @foreach ($gedungs as $gedung)
                            <option value="{{ $gedung->gedung  }}" {{ $check = ($model->exists) ? ($gedung->gedung == $model->room->gedung) ? 'selected' : '' : '' }}>{{ $gedung->description }}</option>
                        @endforeach
                ...
                <div class="col-md-8">
                    <select name="room_code" id="room_code" class="form-control select2bs4" style="width: 100%" {{ $disabled }}>
                        ...
                        @foreach ($rooms as $room)
                            <option value="{{ $room->code  }}" {{ $check = ($model->exists) ? ($room->code == $model->room->code) ? 'selected' : '' : '' }}>{{ $room->name }}</option>
                        @endforeach
                        {{ $disabled }}
                    </select>
...
<script>
    ...
    function getArea() {
        ...
        var gedungCode = $('#gedung_value').val();
        $('#gedung_code').empty();
        $.ajax({
            ...
            success: (data) => {
                ...
                str_data += data.map((gedung) => {
                    if (gedungCode === gedung.gedung) {
                        selected = 'selected';
                    }
                    return '<option value="'+gedung.gedung+'" '+selected+' >'+gedung.description+'</option>';
                });
                $('#gedung_code').append(str_data);
                Swal.close();
                $('#gedung_code').trigger('change');
            },
            error: (xhr) => {
                console.log(xhr);
                Swal.close();
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                str_data += data.map((room) => {
                    ...
                    return '<option value="'+room.code+'" '+selected+' > '+room.name+' ---------------- Has '+room.capacity+' Seats</option>';
