<form method="POST" action="{{ route('adm.store_out') }}">
    @csrf
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="form-group row" style="margin-bottom:unset;">
                ...
                <div class="col-md-6">
                    <label class=" col-form-label" style="font-weight:normal">{{ $nomor_surat_out }}</label>
                    @error('ext')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
            ...
            <div class="form-group row">
                <label for="incoming_mail" class="col-md-4 col-form-label text-md-right">Incoming Mail</label>
                <div class="col-md-6">
                    <select name="incoming_mail" id="incoming_mail" class="form-control select2">
                            <option value="null">--Select Incoming Mail--</option>
                            <option value="0">None</option>
                            @foreach ($incoming_mail as $incoming)
                            <Option Value="{{$incoming->id}}">{{$incoming->client_letter_number}}</option>
                            @endforeach
                    </select>
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-md-6">
                    <textarea type="text" class="form-control" name="attention" id="attention"></textarea>
                    @error('destination')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
...
<script>
    ...
    $('#outgoing_table').DataTable().ajax.reload();
});
</script>
