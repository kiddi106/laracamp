...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">Routers from Purchase Order</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item active"><i class="nav-icon fas fa-table"></i> Detail Router</li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
    ...
    <div class="card">
        ...
        <div class="card-body">
            <dl class="row">
                ...
                @php
                    $detailed = number_format(App\Models\Router::where('po_dtl_id', '=', $poDtl->id)->count(), 0, ',', '.');
                    $good = number_format(App\Models\Router::where('po_dtl_id', '=', $poDtl->id)->where('condition', '=', 'GOOD')->count(), 0, ',', '.');
                    $bad = number_format(App\Models\Router::where('po_dtl_id', '=', $poDtl->id)->where('condition', '=', 'BAD')->count(), 0, ',', '.');
                @endphp
...
<script>
    $(function () {
        ...
        $('#datatable').DataTable({
            ...
            cache: false,
            ajax: {
                url: "{{ route('po.dtl.router.datatable') }}",
                type: 'POST',
                headers: {
            ...
            columns: [
                ...
                {data: 'esn', name: 'esn'},
                {data: 'ssid', name: 'ssid'},
                {data: 'password_router', name: 'password_router'},
                {data: 'guest_ssid', name: 'guest_ssid'},
                {data: 'password_guest', name: 'password_guest'},
                {data: 'password_admin', name: 'password_admin'},
                {data: 'imei', name: 'imei'},
        ...
        const toolbar = '<div class="dt-buttons btn-group flex-wrap">'
            + '<a href="{{ route("po.dtl.router.create", ["po_dtl_id" => $poDtl->id]) }}" class="btn btn-sm btn-default modal-show" title="Add New Router"><i class="fas fa-plus"></i> Add</a>'
        ...
        $('#uploadExcel').on('click', function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('po.dtl.router.upload') }}",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                cache: false,
